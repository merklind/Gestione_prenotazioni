...
def start_reservation_border():
    upper_side_border = Border(top=Side(border_style=BORDER_MEDIUM), left=Side(border_style=BORDER_MEDIUM),
...
def end_reservation_border():
    bottom_border = Border(bottom=Side(border_style=BORDER_MEDIUM), left=Side(border_style=BORDER_MEDIUM),
                           right=Side(border_style=BORDER_MEDIUM))
...
def set_rendiconto_occupation_cell(ws_rendiconto, start_row_rendiconto, info_reservation, occupation_column):
    ...
    if date_check_in != date_check_out - timedelta(days=1):
        ...
        while current_date_cell.value != date_check_out:
            if current_date_cell.value is not None and date_check_in < current_date_cell.value < date_check_out - timedelta(
                    days=1):
...
def set_value_net_cell(ws_rendiconto, current_date_cell, current_cell, channel, gross_column, start_row_rendiconto,
                       offset_date):
...
def set_rendiconto_net_cell(ws_rendiconto, start_row_rendiconto, net_column, gross_column, info_reservation):
    ...
    while current_date_cell.value != date_check_out:
        set_value_net_cell(ws_rendiconto, current_date_cell, current_cell, channel, gross_column, start_row_rendiconto,
                           offset_date)
        offset_date += 1
        current_date_cell = ws_rendiconto.cell(row=start_row_rendiconto + offset_date, column=1)
        current_cell = ws_rendiconto.cell(row=start_row_rendiconto + offset_date, column=net_column)
    ...
    while days_remaining > 0:
        if current_date_cell.value is not None:
            ...
            if info_reservation['Giorni'] <= 31:
                set_value_gross_cell(gross_cell, current_date_cell,
                                     info_reservation['Giorno ' + str(info_reservation['Giorni'] - days_remaining + 1)],
                                     info_reservation['Tipo tariffa'])
            elif info_reservation['Giorni'] > 31:
                set_value_gross_cell(gross_cell, current_date_cell, info_reservation['Importo notte'],
                                     info_reservation['Tipo tariffa'])
            set_value_net_cell(ws_rendiconto, current_date_cell, net_cell, info_reservation['Tramite'], gross_column,
                               start_row_rendiconto, offset_date)
            days_remaining -= 1
            offset_date += 1
            current_date_cell = ws_rendiconto.cell(row=start_row_rendiconto + offset_date, column=1)
        else:
...
def erase_all_reservation_rendiconto(ws_rendiconto: openpyxl.worksheet.worksheet.Worksheet, start_row_rendiconto,
                                     end_row_rendiconto):
    offset_row = 0
    max_col = ws_rendiconto.max_column
    for index in range(start_row_rendiconto, end_row_rendiconto + 1):
        ...
        if current_cell.value is not None:
            for cell in ws_rendiconto.iter_cols(min_col=3, max_col=max_col,
                                                min_row=start_row_rendiconto + offset_row,
                                                max_row=start_row_rendiconto + offset_row):
                erase_cell(cell[0])
        offset_row += 1
...
def erase_future_reservation(ws_rendiconto, today, start_row_rendiconto, end_row_rendiconto):
    ...
    for index in range(start_row_rendiconto, end_row_rendiconto + 1):
        ...
        if current_cell.value is not None:
            ...
            for cell in ws_rendiconto.iter_cols(min_col=3, max_col=max_col,
                                                min_row=start_row_rendiconto + offset_date,
